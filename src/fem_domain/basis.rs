...
/// One Directional Component of a Higher-Order Vectorial Hierarchical Basis Function Space: defined on (-1.0, +1.0)
///
/// Outer products of the 1D vectors in pairs of HierBasisFnSpaces are used to define 2D Basis Functions ([HierBasisFnSampled]) via the [BasisFnSampler]
///
...
///
/// For example: a u-directed basis function with orders (2, 3) will sample `u_shape.tang(2, m) * v_shape.norm(3, n)`, and a v-directed basis function with orders (4, 1) will sample `u_shape.norm(4, m) * v_shape.tang(1, n)` for all points `m` and `n`
///
...
///
pub struct BasisFnSampler<BSpace: HierBasisFnSpace> {
    ...
    computed: Arc<Mutex<HashMap<BSDescription, Arc<HierBasisFnSampled<BSpace>>>>>,
}

impl<BSpace: HierBasisFnSpace> BasisFnSampler<BSpace> {
    ...
    /// * `compute_2nd_derivs`: whether or not to compute the second derivatives of the [HierBasisFnSpace]s.
    pub fn with(
        i_max: usize,
        j_max: usize,
    ...
    /// Generate or retrieve a [HierBasisFnSampled] defined over an [Elem]. Can be defined over a subset of the Element.
    pub fn sample_basis_fn(
        &mut self,
        elem: &Elem,
        over_desc_elem: Option<&Elem>,
    ) -> Arc<HierBasisFnSampled<BSpace>> {
        ...
        match self.computed.lock() {
            Ok(mut comp_guard) => {
                if let Some(computed_bs) = comp_guard.get(&desc) {
                    ...
                    let bs = HierBasisFnSampled::mapped_over_desc(
                        self.i_max,
                        self.j_max,
                        self.compute_d2,
                        &self.u_points,
            ...
            Err(_) => Arc::new(HierBasisFnSampled::mapped_over_desc(
                self.i_max,
                self.j_max,
                self.compute_d2,
                &self.u_points,
...
impl<BSpace: HierBasisFnSpace> Clone for BasisFnSampler<BSpace> {
    fn clone(&self) -> Self {
        Self {
            i_max: self.i_max,
            j_max: self.j_max,
...
/// Structure used to evaluate Basis Functions (and their derivatives) from a [HierBasisFnSpace]'s over some area
#[derive(Clone, Debug)]
pub struct HierBasisFnSampled<SF: HierBasisFnSpace> {
